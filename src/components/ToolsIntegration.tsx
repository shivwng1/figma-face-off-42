  ...
  return (
    <section id="integrations" className="relative flex w-full max-w-[1240px] flex-col items-center mt-[130px] max-md:max-w-full max-md:mt-10">
      ...
      <div className="flex min-h-36 w-full flex-col items-center mt-[50px] max-md:max-w-full max-md:mt-10">
        <div className="flex w-full items-center gap-5 justify-center max-md:max-w-full">
          {tools.map((tool, index) => (
            <div
              key={index}
              className={`bg-[rgba(255,255,255,0.02)] border self-stretch flex min-h-36 flex-col items-center justify-center w-[200px] my-auto px-6 py-10 rounded-[20px] border-[rgba(255,255,255,0.1)] border-solid max-md:px-5 ${
                index === tools.length - 1 ? 'pl-6 pr-2.5' : ''
              }`}
            >
              <img
                src={tool.src}
                className={`${tool.aspect} object-contain ${tool.width}`}
